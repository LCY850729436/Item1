...
//将文本里的所有字符转化为7位二进制数据，并在最后添加7位全为0的数据作为结束标志位
...
void DrawQRcode() 
{
	...
	while (size != 0)
	{
		...
		//所有数据全部绘制进二维码后，若二维码没被填满则将其用灰色和白色交替填满便于定位识别
		if (size == 0)
		{
			y = y + 12;
			//将没填满的一行填满
...
string ImageToVideo()
{
	...
	string str3 = " 视频\\";
	finalInstrcut = str1 + "图片\\%d.jpg " + str2 + VideoLen + str3 + Name;
...
int main()
{
	...
	cout << endl << "已将二进制文件编码为图片，请按照提示输入要编码的视频的名字和帧率" << endl;
	cout << endl;
