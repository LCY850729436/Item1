...
{
	//再对int类容器里的数据进行还原
	ofstream fout("解码后的二进制文件//decode.bin");
	int flag = 1;//判断是否读到了结束标志，即连续读到7个0
	for (int i = 0;i < decodeByte.size() && flag == 1;i = i + 7)
	{
		int byte[7] = { 0 };
		int count = 0;
		for (int j = 0;j < 7;j++)
		...
		{
			int num = 0;
			for (int j = 6;j >= 0;j--)num = num + byte[j] * pow(2, 6 - j);
			char ch = (char)num;
			fout << ch;
			if (i + 7 >= decodeByte.size())break;
		}
	}
}
...
int main()
{
	...
	else
	{
		...
		cout << endl << "已完成解码，已经生成decode.bin文件" << endl;
	}
